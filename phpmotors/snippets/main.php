<h1 class="title">Welcome to PHP Motors!</h1>
<div class="car_info">
    <div class="card">
        <h3>
            DMC Delorean
        </h3>
        <ul>
            <li>3 Cup holders</li>
            <li>Superman doors</li>
            <li>Fuzzy dice!</li>
        </ul>
    </div>
    <img class="car" src="/phpmotors/images/delorean.jpg" alt="">
    <div class="own_today">
        <img src="/phpmotors/images/site/own_today.png" alt="">
    </div>
</div>

<div class="reviews-upgrades">
    <div class="reviews">
        <h1 class="title">DMC Delorean Reviews</h1>
        <ul class="comments">
            <li>"So fast its almost like traveling in time." [4/5]</li>
            <li>"Coolest ride on the road." [4/5]</li>
            <li>"I'm feeling Marty McFly!" [5/5]</li>
            <li>"The most futuristic ride of our day." [4.5/5]</li>
            <li>"80's living and love it." [5/5]</li>
        </ul>
    </div>

    <div class="upgrades-grid">
        <h1 class="title-upgrades">Delorean Upgrades</h1>
        <div class="upgrades">
            <div>
                <div>
                    <img src="/phpmotors/images/upgrades/bumper_sticker.jpg" alt="Bumper Sticker" width="65" height="65">
                </div>
                <p><a href="/phpmotors/views/home.php">Bumper Sticker</a></p>
            </div>
            <div>
                <div>
                    <img src="/phpmotors/images/upgrades/flame.jpg" alt="Flames Decals" width="65" height="65">
                </div>
                <p><a href="/phpmotors/views/home.php">Flames Decals</a></p>
            </div>
            <div>
                <div>
                    <img src="/phpmotors/images/upgrades/flux-cap.png" alt="Flux Capacitor" width="65" height="65">
                </div>
                <p><a href="/phpmotors/views/home.php">Flux Capacitor</a></p>
            </div>
            <div>
                <div> 
                    <img src="/phpmotors/images/upgrades/hub-cap.jpg" alt="Hub Caps" width="65" height="65">
                </div>
                <p><a href="/phpmotors/views/home.php">Hub Caps</a></p>
            </div>
        </div>
    </div>
</div>